...
export class UserLimitationsController {
  constructor(
    private readonly userLimitationsService: UserLimitationsService,
  ) {}
  ...
  create(
    @GetUser() user: User,
    @Body() createUserLimitationDto: CreateUserLimitationDto,
  ) {
  ...
  getFoodPlan(@Param('type') type: string) {
    if (['vegano', 'vegetariano', 'curdivegano', 'animal'].includes(type)) {
      ...
      return {
        ok: false,
        message: 'los tipos deben son vegano,vegetariano,curdivegano,animal',
      };
    }
  }
}
